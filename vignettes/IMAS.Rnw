...
We provide example data to explain the manual of the \Biocpkg{IMAS} package. The dataset contains 50 individuals with breast cancer, half of whom are assigned as PR-positive, with the other half PR-negative. Our assumption is that the exon, located in the PRGA gene on chr11:100962491-100962607, has differential PSI values for the two groups and that the difference is associated with SNPs or methylation in the exon and its flanking introns. The RNA-seq, SNP, methylation, and clinical data for this example are provided in the \Biocpkg{IMAS} package.
...
ASdb <- sQTLsFinder(ASdb=ASdb,Total.snpdata=samplesnp,Total.snplocus=samplesnplocus,
    GroupSam=GroupSam,Ncor=1,CalIndex="ES3",method="lm")
ASdb
...
\end{center}
This plot shows the sQTL results. For each group, the distribution of PSI values across genotypes is shown as a boxplot.
